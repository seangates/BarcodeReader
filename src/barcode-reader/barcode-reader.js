...
function BarcodeReader(config) {
  this.config = {
    ...
    decodeFormats: config.decodeFormats || DEFAULT_FORMATS,
...
BarcodeReader.prototype = {
  SupportedFormats: DEFAULT_FORMATS, // Don't touch.
  ...
  DecoderWorker: new Worker(
    URL.createObjectURL(
      new Blob([require('./decoder-worker')], {
  ...
  BarcodeReaderImageCallback: function(e) {
    ...
    for (var i = 0; i < e.data.result.length; i++) {
      if (this.Decoded.indexOf(e.data.result[i].Value) === -1 || this.config.forceUnique === false) {
        ...
        if (this.config.forceUnique) {
          this.Decoded.push(e.data.result[i].Value);
        }
      }
    }
